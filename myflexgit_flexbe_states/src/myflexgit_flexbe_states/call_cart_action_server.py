...
class CallCart(EventState):

    '''
    Calls CartLinTaskAction server @ 'joint_trap_vel_action_server' topic
    [...]
    ...
    def __init__(self,max_vel,max_acl,namespace=''):
        super(CallCart, self).__init__(outcomes = ['continue', 'failed'], input_keys = ['translation_move'], output_keys = ['joint_values'])

        # replace 'panda_2/joint_trap_vel_action_server' from dummies with 'joint_trap_vel_action_server'
        self._namespace=namespace
        
        self._topic = self._namespace + '/cart_lin_action_server'
    ...
    def on_enter(self, userdata):
        ...
        self.goal_joint = userdata.translation_move #JointState()
        joint = JointState()
        joint.position = self.goal_joint
        goal = CartLinTaskGoal([joint], self.max_vel, self.max_acl)
...
if __name__ == '__main__':
    ...
    test_state=CallCart(0.5,0.1,namespace='')
